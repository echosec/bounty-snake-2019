...
resource "digitalocean_droplet" "bounty_snake_droplet" {
  ...
  user_data   = <<EOM
    ...
    runcmd:
      ...
      - docker run -t -d -p 80:5000 --env VERSION=${var.image_tag} --env REDIS_HOST=${digitalocean_droplet.bounty_snake_redis.ipv4_address} --restart=unless-stopped --log-driver=syslog --log-opt syslog-address=udp://${var.papertrail_url} --log-opt tag="bounty-snake-${var.image_tag}" docker.pkg.github.com/echosec/bounty-snake-2020/bounty-snake-2020:${var.image_tag}
    EOM
